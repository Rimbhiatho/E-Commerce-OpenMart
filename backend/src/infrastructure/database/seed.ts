  ...
  try {
    ...
    if (!existingAdmin) {
      ...
      await db.run(
        `INSERT INTO users (id, email, password, name, role, balance, createdAt, updatedAt) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [adminId, adminEmail, hashedPassword, 'Admin OpenMart', 'admin', 100000000, now, now]
      );
    ...
    if (!existingCustomer) {
      ...
      await db.run(
        `INSERT INTO users (id, email, password, name, role, balance, createdAt, updatedAt) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [customerId, customerEmail, hashedPassword, 'Customer OpenMart', 'customer', 100000000, now, now]
      );
      ...
      if (!existingCat) {
        ...
        await db.run(
          `INSERT INTO categories (id, name, description, createdAt, updatedAt) 
           VALUES (?, ?, ?, ?, ?)`,
          [uuidv4(), cat.name, cat.description, now, now]
        );
